...
function normalizeUrl(input: string) {
  ...
  try {
    ...
    throw new Error("URL inválida. Ejemplo: https://mi-tienda.com");
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!url || typeof url !== "string") {
      return new Response(JSON.stringify({ error: "Falta 'url' en el body." }), { status: 400 });
    }
    ...
    const findings: Finding[] = [
      ...
      },
      {
        ...
        message: toBoolHeader(xcto)
          ? "X-Content-Type-Options presente"
          : "X-Content-Type-Options ausente",
        advice: toBoolHeader(xcto)
          ? undefined
    ...
    // 5) Score sencillo
    let score =
      findings.filter(f => f.ok && f.key !== "httpsEnabled" && f.key !== "httpRedirectsToHttps").length *
      10;
    ...
    const priorities = findings
      ...
      .slice(0, 3)
      .map(f => ({ key: f.key, advice: f.advice }));

    return new Response(
      JSON.stringify(
        { host, httpsEnabled, httpRedirectsToHttps, score, findings, priorities },
        null,
        2
      ),
